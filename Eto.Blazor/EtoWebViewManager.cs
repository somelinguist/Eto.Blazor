...
{
    public class EtoWebViewManager : WebViewManager
    {
        private readonly WebViewWithMessaging _webView;
        ...
        public EtoWebViewManager(WebViewWithMessaging webview, IServiceProvider provider, Dispatcher dispatcher, Uri appBaseUri, IFileProvider fileProvider, JSComponentConfigurationStore jsComponents, string hostPageRelativePath)
            : base(provider, dispatcher, appBaseUri, fileProvider, jsComponents, hostPageRelativePath)
        {
            ...
            _webView.WebMessageReceived += (sender, message) =>
            {
                MessageReceived(message.Source, message.Message);
                ...
                //
                //     MessageReceived(msg., (string)message!);
        ...
        {
            _webView.Url = absoluteUri;
        }

        protected override void SendMessage(string message)
        {
            Task.Run(() => Dispatcher.InvokeAsync(() => _webView.ExecuteScriptAsync(message)));
        }
    }
}